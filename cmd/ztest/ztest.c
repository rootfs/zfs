...
{
	void *buf = abd_borrow_linear(abd, size);
	uint64_t *ip = buf;
	...
	abd_return_linear_copy(abd, buf, size);
}
...
ztest_bt_bonus(dmu_buf_t *db)
{
	...
	bt = (void *)((char *)ABD_TO_LINEAR(db->db_data) + doi.doi_bonus_size - sizeof (*bt));
		...
		for (off = bigoff, j = 0; j < s; j++, off += chunksize) {
			...
			if (i != 5) {
				abd_copy_from_buf(bigbuf_arcbufs[j]->b_data,
				    (caddr_t)bigbuf + (off - bigoff), chunksize);
			} else {
				abd_copy_from_buf(bigbuf_arcbufs[2 * j]->b_data,
				    (caddr_t)bigbuf + (off - bigoff),
				    chunksize / 2);
				abd_copy_from_buf(bigbuf_arcbufs[2 * j + 1]->b_data,
				    (caddr_t)bigbuf + (off - bigoff) + chunksize / 2,
				    chunksize / 2);
			}
...
ztest_ddt_repair(ztest_ds_t *zd, uint64_t id)
{
	...
	ztest_pattern_set(LINEAR_TO_ABD(buf), psize, ~pattern);

	(void) zio_wait(zio_rewrite(NULL, spa, 0, &blk,
	    LINEAR_TO_ABD(buf), psize, NULL, NULL, ZIO_PRIORITY_SYNC_WRITE,
