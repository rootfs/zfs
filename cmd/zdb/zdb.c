	...
	if (object == 0) {
		...
		if (error)
			fatal("dmu_bonus_hold(%llu) failed, errno %u",
			    object, error);
		bonus = ABD_TO_LINEAR(db->db_data);
		bsize = db->db_size;
...
dump_config(spa_t *spa)
{
	...
	if (error == 0) {
		nvsize = *(uint64_t *)ABD_TO_LINEAR(db->db_data);
		dmu_buf_rele(db, FTAG);
...
zdb_read_block(char *thing, spa_t *spa)
{
	...
	if (vd == vd->vdev_top) {
		 ...
		 */
		zio_nowait(zio_read(zio, spa, bp, LINEAR_TO_ABD(pbuf), psize, NULL, NULL,
		 ...
		 */
		zio_nowait(zio_vdev_child_io(zio, bp, vd, offset, LINEAR_TO_ABD(pbuf), psize,
