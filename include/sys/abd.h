...
({							\
	type ret;					\
	if (ABD_IS_LINEAR(abd))				\
		ret = u_##func(__VA_ARGS__);		\
	else						\
		ret = do_##func(__VA_ARGS__);		\
	ret;						\
})
...
/*
 * ABD functions for scatter buffer
 * Should not be used directly
 */
void do_abd_iterate_rfunc(abd_t *, size_t, int (*)(const void *, uint64_t, void *), void *);
void do_abd_iterate_wfunc(abd_t *, size_t, int (*)(void *, uint64_t, void *), void *);
void do_abd_iterate_func2(abd_t *, abd_t *, size_t, size_t, int (*)(void *, void *, uint64_t, uint64_t, void *), void *);
...
({						\
	void *b;				\
	b = zio_buf_alloc(n);			\
	do_abd_copy_to_buf_off(b, a, n, 0);	\
	b;					\
})
#define do_abd_return_linear(a, b, n) zio_buf_free(b, n)
#define do_abd_return_linear_copy(a, b, n)	\
do {						\
 ...
 * Should not be used directly
 */
#define u_abd_iterate_rfunc(a, n, f, p)		(void)f(a, n, p)
#define u_abd_iterate_wfunc(a, n, f, p)		(void)f(a, n, p)
#define u_abd_iterate_func2(a, b, an, bn, f, p)	(void)f(a, b, an, bn, p)
#define u_abd_copy(a, b, n)			(void)memcpy(a, b, n)
#define u_abd_copy_from_buf_off(a, b, n, off)	(void)memcpy((void *)(a)+(off), b, n)
#define u_abd_copy_to_buf_off(a, b, n, off)	(void)memcpy(a, (void *)(b)+(off), n)
...
#endif
#define u_abd_borrow_linear(a, n)		((void *)a)
#define u_abd_borrow_linear_copy(a, n)		((void *)a)
#define u_abd_return_linear(a, b, n)		do { } while (0)
#define u_abd_return_linear_copy(a, b, n)	do { } while (0)
...
#define abd_iterate_func2(dabd, sabd, dsize, ssize, func2, priv) \
	ABD_FUNC2_WRAPPER(dabd, sabd, abd_iterate_func2, dabd, sabd, dsize, ssize, func2, priv)
...
#define abd_copy_to_user_off(buf, abd, size, off) \
	ABD_RET_FUNC_WRAPPER(int, abd, abd_copy_to_user_off, buf, abd, size, off)
...
#define abd_copy_from_user_off(abd, buf, size, off) \
	ABD_RET_FUNC_WRAPPER(int, abd, abd_copy_from_user_off, abd, buf, size, off)
 ...
 */
#define abd_borrow_linear(abd, size) \
	ABD_RET_FUNC_WRAPPER(void *, abd, abd_borrow_linear, abd, size)

/*
 * Borrow a linear buffer for an ABD
 * Will allocate and copy if ABD is scatter
 */
#define abd_borrow_linear_copy(abd, size) \
	ABD_RET_FUNC_WRAPPER(void *, abd, abd_borrow_linear_copy, abd, size)

/*
 * Return the linear buffer
 */
#define abd_return_linear(abd, buf, size) \
	ABD_FUNC_WRAPPER(abd, abd_return_linear, abd, buf, size)

/*
 * Copy back to ABD and return the linear buffer
 */
#define abd_return_linear_copy(abd, buf, size) \
	ABD_FUNC_WRAPPER(abd, abd_return_linear_copy, abd, buf, size)
