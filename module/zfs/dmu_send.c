    ...
    const zbookmark_t *zb, const dnode_phys_t *dnp, void *arg)
{
	...
	if (zb->zb_object != DMU_META_DNODE_OBJECT &&
		...
		err = dump_spill(dsp, zb->zb_object, blksz, ABD_TO_LINEAR(abuf->b_data));
		(void) arc_buf_remove_ref(abuf, &abuf);
		...
		if (arc_read(NULL, spa, bp, arc_getbuf_func, &abuf,
			...
			if (zfs_send_corrupt_data) {
				...
				abd_iterate_wfunc(abuf->b_data, blksz, fillbadblock, NULL);
			} else {
				return (SET_ERROR(EIO));
			}
		}

		buf = abd_borrow_linear_copy(abuf->b_data, blksz);
		err = dump_data(dsp, type, zb->zb_object, zb->zb_blkid * blksz,
		    blksz, bp, buf);
		abd_return_linear(abuf->b_data, buf, blksz);
		(void) arc_buf_remove_ref(abuf, &abuf);
	}
...
restore_object(struct restorearg *ra, objset_t *os, struct drr_object *drro)
{
	...
	if (data != NULL) {
		...
		bcopy(data, ABD_TO_LINEAR(db->db_data), drro->drr_bonuslen);
		if (ra->byteswap) {
			dmu_object_byteswap_t byteswap =
			    DMU_OT_BYTESWAP(drro->drr_bonustype);
			dmu_ot_byteswap[byteswap].ob_func(ABD_TO_LINEAR(db->db_data),
			    drro->drr_bonuslen);
		}
    ...
    struct drr_write_byref *drrwbr)
{
	...
	buf = abd_borrow_linear_copy(dbp->db_data, drrwbr->drr_length);
	dmu_write(os, drrwbr->drr_object,
	    drrwbr->drr_offset, drrwbr->drr_length, buf, tx);
	abd_return_linear(dbp->db_data, buf, drrwbr->drr_length);
