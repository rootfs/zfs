...
zio_pop_transforms(zio_t *zio)
{
	...
	while ((zt = zio->io_transform_stack) != NULL) {
		...
		if (zt->zt_bufsize != 0) {
			if (ABD_IS_LINEAR(zio->io_data))
				zio_buf_free(ABD_TO_LINEAR(zio->io_data),
				    zt->zt_bufsize);
			else
...
zio_decompress(zio_t *zio, abd_t *data, uint64_t size)
{
	...
	if (zio->io_error == 0) {
		...
		buf1 = abd_borrow_linear_copy(zio->io_data, zio->io_size);
		buf2 = abd_borrow_linear(data, size);

		if (zio_decompress_data(BP_GET_COMPRESS(zio->io_bp),
		    buf1, buf2, zio->io_size, size) != 0)
			zio->io_error = SET_ERROR(EIO);

		abd_return_linear_copy(data, buf2, size);
		abd_return_linear(zio->io_data, buf1, zio->io_size);
	}
}
    ...
    zio_priority_t priority, enum zio_flag flags, boolean_t labels)
{
	...
	if (zio_checksum_table[checksum].ci_eck) {
		...
		if (ABD_IS_LINEAR(data))
			wbuf = LINEAR_TO_ABD(zio_buf_alloc(size));
		else
			wbuf = abd_alloc(size);
...
zio_read_bp_init(zio_t *zio)
{
	...
	if (BP_GET_COMPRESS(bp) != ZIO_COMPRESS_OFF &&
		...
		abd_t *cbuf;
		if (ABD_IS_LINEAR(zio->io_data))
			cbuf = LINEAR_TO_ABD(zio_buf_alloc(psize));
		else
...
zio_write_bp_init(zio_t *zio)
{
	...
	if (compress != ZIO_COMPRESS_OFF) {
		...
		dbuf = abd_borrow_linear_copy(zio->io_data, lsize);
		psize = zio_compress_data(compress, dbuf, cbuf, lsize);
		abd_return_linear(zio->io_data, dbuf, lsize);
...
static zio_t *
zio_read_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn, abd_t *data, uint64_t offset)
{
	if (gn != NULL)
		return (pio);
...
static zio_t *
zio_rewrite_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn, abd_t *data, uint64_t offset)
{
	zio_t *zio;

	if (gn != NULL) {
		zio = zio_rewrite(pio, pio->io_spa, pio->io_txg, bp,
		    LINEAR_TO_ABD(gn->gn_gbh), SPA_GANGBLOCKSIZE, NULL, NULL, pio->io_priority,
...
/* ARGSUSED */
static zio_t *
zio_free_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn, abd_t *data, uint64_t offset)
{
...
/* ARGSUSED */
static zio_t *
zio_claim_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn, abd_t *data, uint64_t offset)
{
...
zio_gang_tree_assemble(zio_t *gio, blkptr_t *bp, zio_gang_node_t **gnpp)
{
	...
	zio_nowait(zio_read(gio, gio->io_spa, bp, LINEAR_TO_ABD(gn->gn_gbh),
...
zio_gang_tree_assemble_done(zio_t *zio)
{
	...
	if (BP_SHOULD_BYTESWAP(bp))
		byteswap_uint64_array(ABD_TO_LINEAR(zio->io_data), zio->io_size);

	ASSERT(zio->io_data == LINEAR_TO_ABD(gn->gn_gbh));
...
static void
zio_gang_tree_issue(zio_t *pio, zio_gang_node_t *gn, blkptr_t *bp, abd_t *data, uint64_t offset)
{
...
zio_gang_issue(zio_t *zio)
{
	...
	if (zio->io_child_error[ZIO_CHILD_GANG] == 0)
		zio_gang_tree_issue(zio, zio->io_gang_tree, bp, zio->io_data, 0);
	else
...
zio_write_gang_block(zio_t *pio)
{
	 ...
	 */
	zio = zio_rewrite(pio, spa, txg, bp, LINEAR_TO_ABD(gbh), SPA_GANGBLOCKSIZE, NULL, NULL,
...
zio_ddt_child_read_done(zio_t *zio)
{
	...
	if (zio->io_error == 0 && dde->dde_repair_data == NULL)
		dde->dde_repair_data = ABD_TO_LINEAR(zio->io_data);
	else
	...
	if (zio->io_child_error[ZIO_CHILD_DDT]) {
		...
		for (p = 0; p < DDT_PHYS_TYPES; p++, ddp++) {
			...
			zio_nowait(zio_read(zio, zio->io_spa, &blk,
			    LINEAR_TO_ABD(zio_buf_alloc(zio->io_size)), zio->io_size,
...
zio_vdev_io_start(zio_t *zio)
{
	...
	if (P2PHASE(zio->io_size, align) != 0) {
		...
		abd_t *abuf;
		if (ABD_IS_LINEAR(zio->io_data))
			abuf = LINEAR_TO_ABD(zio_buf_alloc(asize));
		else
	...
	if (zio->io_error == 0) {
		while (zio->io_cksum_report != NULL) {
			...
			if (asize != zio->io_size) {
				...
				abd_copy_to_buf(abuf, zio->io_data, zio->io_size);
				bzero(abuf+zio->io_size, asize-zio->io_size);
			} else {
				abuf = abd_borrow_linear_copy(zio->io_data, zio->io_size);
			}
			...
			if (asize != zio->io_size)
				...
				abd_return_linear(zio->io_data, abuf, zio->io_size);
		}
	}
