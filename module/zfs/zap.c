...
fzap_upgrade(zap_t *zap, dmu_tx_t *tx, zap_flags_t flags)
{
	 ...
	 */
	bzero(ABD_TO_LINEAR(zap->zap_dbuf->db_data), zap->zap_dbuf->db_size);
	...
	l->l_dbuf = db;
	l->l_phys = ABD_TO_LINEAR(db->db_data);
    ...
    dmu_tx_t *tx)
{
	...
	transfer_func(ABD_TO_LINEAR(db_old->db_data),
	    ABD_TO_LINEAR(db_new->db_data), hepb);
	...
	dmu_buf_will_dirty(db_new, tx);
	transfer_func((uint64_t *)ABD_TO_LINEAR(db_old->db_data) + hepb,
	    ABD_TO_LINEAR(db_new->db_data), hepb);
	dmu_buf_rele(db_new, FTAG);
    ...
    dmu_tx_t *tx)
{
	...
	if (tbl->zt_nextblk != 0) {
		...
		((uint64_t *)ABD_TO_LINEAR(db2->db_data))[off2] = val;
		((uint64_t *)ABD_TO_LINEAR(db2->db_data))[off2+1] = val;
		dmu_buf_rele(db2, FTAG);
	}
	((uint64_t *)ABD_TO_LINEAR(db->db_data))[off] = val;
	dmu_buf_rele(db, FTAG);
...
zap_table_load(zap_t *zap, zap_table_phys_t *tbl, uint64_t idx, uint64_t *valp)
{
	...
	if (err)
		return (err);
	*valp = ((uint64_t *)ABD_TO_LINEAR(db->db_data))[off];
	dmu_buf_rele(db, FTAG);
    ...
    zap_leaf_t **lp)
{
	...
	ASSERT3P(l->l_phys, ==, ABD_TO_LINEAR(l->l_dbuf->db_data));
...
zap_deref_leaf(zap_t *zap, uint64_t h, dmu_tx_t *tx, krw_t lt, zap_leaf_t **lp)
{
	...
	ASSERT(zap->zap_dbuf == NULL ||
	    zap->zap_f.zap_phys == ABD_TO_LINEAR(zap->zap_dbuf->db_data));
...
fzap_get_stats(zap_t *zap, zap_stats_t *zs)
{
	...
	if (zap->zap_f.zap_phys->zap_ptrtbl.zt_numblks == 0) {
		...
		for (b = 0; b < zap->zap_f.zap_phys->zap_ptrtbl.zt_numblks;
			...
			if (err == 0) {
				zap_stats_ptrtbl(zap, ABD_TO_LINEAR(db->db_data),
				    1<<(bs-3), zs);
				dmu_buf_rele(db, FTAG);
			}
		}
