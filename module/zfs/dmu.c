 ...
 */
static int
dmu_req_copy(abd_t *db_data, int db_offset, int size, struct request *req, size_t req_offset)
{
...
dmu_read_uio(objset_t *os, uint64_t object, uio_t *uio, uint64_t size)
{
	...
	for (i = 0; i < numbufs; i++) {
		...
		if (xuio) {
			...
			err = abd_uiomove_off(db->db_data, tocpy, UIO_READ, uio, bufoff);
		}
		if (err)
			break;
...
dmu_write_uio_dnode(dnode_t *dn, uio_t *uio, uint64_t size, dmu_tx_t *tx)
{
	...
	for (i = 0; i < numbufs; i++) {
		 ...
		 * block.
		 */
		err = abd_uiomove_off(db->db_data, tocpy, UIO_WRITE, uio, bufoff);
    ...
    dmu_tx_t *tx)
{
	...
	if (offset == db->db.db_offset && blksz == db->db.db_size) {
		...
		tmp_buf = abd_borrow_linear_copy(buf->b_data, blksz);

		dmu_write(os, object, offset, blksz, tmp_buf, tx);

		abd_return_linear(buf->b_data, tmp_buf, blksz);
