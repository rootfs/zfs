...
vdev_cache_hit(vdev_cache_t *vc, vdev_cache_entry_t *ve, zio_t *zio)
{
	...
	ve->ve_hits++;
	abd_copy_from_buf(zio->io_data, ve->ve_data + cache_phase, zio->io_size);
}
...
vdev_cache_fill(zio_t *fio)
{
	...
	ASSERT(ve->ve_data == ABD_TO_LINEAR(fio->io_data));
...
vdev_cache_read(zio_t *zio)
{
	...
	fio = zio_vdev_delegated_io(zio->io_vd, cache_offset,
	    LINEAR_TO_ABD(ve->ve_data), VCBS, ZIO_TYPE_READ, ZIO_PRIORITY_NOW,
