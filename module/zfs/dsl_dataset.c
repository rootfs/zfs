...
dsl_dataset_get_snapname(dsl_dataset_t *ds)
{
	...
	if (err != 0)
		return (err);
	headphys = ABD_TO_LINEAR(headdbuf->db_data);
    ...
    dsl_dataset_t **dsp)
{
	...
	if (ds == NULL) {
		...
		ds->ds_dbuf = dbuf;
		ds->ds_object = dsobj;
		ds->ds_phys = ABD_TO_LINEAR(dbuf->db_data);
	...
	ASSERT3P(ds->ds_phys, ==, ABD_TO_LINEAR(dbuf->db_data));
    ...
    uint64_t flags, dmu_tx_t *tx)
{
	...
	dsphys = ABD_TO_LINEAR(dbuf->db_data);
