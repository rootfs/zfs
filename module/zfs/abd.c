...
({								\
	...
	abd = (arc_buf_data_t *)(((unsigned long)__abd) & ~0x1);\
    ...
    int (*func)(const void *, uint64_t, void *), void *private)
{
	...
	sg_miter_start(&miter, &abd->abd_sgl[skip_pn], n - skip_pn, ABD_MITER_RFLAGS);
    ...
    int (*func)(void *, uint64_t, void *), void *private)
{
	...
	sg_miter_start(&miter, &abd->abd_sgl[skip_pn], n - skip_pn, ABD_MITER_WFLAGS);
    ...
    int (*func2)(void *, void *, uint64_t, uint64_t, void *), void *private)
{
	...
	while(dsize > 0 || ssize > 0) {
		...
		/* both have remainings after this run, use equal len */
...
void
do_abd_copy_from_buf_off(abd_t *__abd, const void *buf, size_t size, size_t off)
{
	size_t len;
	int n;
	int skip_pn, skip_off;
...
int
do_abd_copy_from_user_off(abd_t *__abd, const void __user *buf, size_t size, size_t off)
{
	int i, ret = 0;
	size_t len;
	int skip_pn, skip_off;
...
int
do_abd_uiomove_off(abd_t *__abd, size_t n, enum uio_rw rw, uio_t *uio, size_t off)
{
	struct iovec *iov;
	ulong_t cnt;
...
int
do_abd_uiocopy_off(abd_t *__abd, size_t n, enum uio_rw rw, uio_t *uio, size_t *cbytes, size_t off)
{
	struct iovec *iov;
	ulong_t cnt;
	int iovcnt;
...
unsigned int
do_abd_bio_map_off(struct bio *bio, abd_t *__abd, unsigned int bio_size, size_t off)
{
	int i;
	size_t len;
	int skip_pn, skip_off;
	...
	for (i = 0; i < bio->bi_max_vecs; i++) {
		...
		if (bio_add_page(bio, sg_page(&abd->abd_sgl[skip_pn+i]), len, skip_off) != len)
			break;
...
abd_alloc(size_t size)
{
	...
	abd = kmem_alloc(sizeof(arc_buf_data_t)+n*sizeof(struct scatterlist), KM_PUSHPAGE);
...
abd_free(abd_t *__abd, size_t size)
{
	...
	kmem_free(abd, sizeof(arc_buf_data_t)+n*sizeof(struct scatterlist));
}
