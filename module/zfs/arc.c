...
>>>>>>> 99eb0b7b9c26c05a47cc369b20066032bfb6240c
#else
...
{
	return LINEAR_TO_ABD(zio_buf_alloc(size));
}

static void abd_meta_free(abd_t *buf, size_t size)
{
	zio_buf_free(ABD_TO_LINEAR(buf), size);
}
...
=======
			arc_space_return(sizeof (*l2hdr), ARC_SPACE_L2HDRS);
...
arc_read_done(zio_t *zio)
{
	...
	if (BP_SHOULD_BYTESWAP(zio->io_bp) && zio->io_error == 0) {
		...
		void *ziobuf = abd_borrow_linear_copy(zio->io_data, zio->io_size);
		...
		abd_return_linear_copy(zio->io_data, ziobuf, zio->io_size);
	}
...
top:
	...
	if (hdr && hdr->b_datacnt > 0) {
		...
		*arc_flags |= ARC_CACHED;
		...
		arc_callback_t	*acb;
		vdev_t *vd = NULL;
		uint64_t addr = 0, asize = 0;
		enum zio_compress compress = ZIO_COMPRESS_OFF;
...
=======
			asize = hdr->b_l2hdr->b_asize;
			compress = hdr->b_l2hdr->b_compress;
>>>>>>> 99eb0b7b9c26c05a47cc369b20066032bfb6240c
			/*
...
=======
				if (compress == ZIO_COMPRESS_EMPTY) {
>>>>>>> 99eb0b7b9c26c05a47cc369b20066032bfb6240c
...
=======
					    asize, buf->b_data,
					    ZIO_CHECKSUM_OFF, l2arc_read_done,
					    cb, priority, zio_flags |
					    ZIO_FLAG_DONT_CACHE |
>>>>>>> 99eb0b7b9c26c05a47cc369b20066032bfb6240c
					    ZIO_FLAG_CANFAIL |
					    ZIO_FLAG_DONT_PROPAGATE |
...
=======
				ARCSTAT_INCR(arcstat_l2_read_bytes, asize);
>>>>>>> 99eb0b7b9c26c05a47cc369b20066032bfb6240c
	...
	for (try = 0; try <= 3; try++) {
		...
		for (; ab; ab = ab_prev) {
			...
			if (pio == NULL) {
				...
				cb = kmem_zalloc(sizeof (l2arc_write_callback_t),
				                KM_PUSHPAGE);
				cb->l2wcb_dev = dev;
				cb->l2wcb_head = head;
...
l2arc_compress_buf(l2arc_buf_hdr_t *l2hdr)
{
	...
	ddata = abd_borrow_linear_copy(l2hdr->b_tmp_cdata, l2hdr->b_asize);

	csize = zio_compress_data(ZIO_COMPRESS_LZ4, ddata,
	    cdata, l2hdr->b_asize);

	abd_return_linear(l2hdr->b_tmp_cdata, ddata, l2hdr->b_asize);
	...
	if (csize == 0) {
		...
		l2hdr->b_asize = csize;
		l2hdr->b_tmp_cdata = LINEAR_TO_ABD(cdata);
...
l2arc_decompress_zio(zio_t *zio, arc_buf_hdr_t *hdr, enum zio_compress c)
{
	...
	if (c == ZIO_COMPRESS_EMPTY) {
		...
		ddata = abd_borrow_linear(zio->io_data, hdr->b_size);

		if (zio_decompress_data(c, cdata, ddata, csize,
		    hdr->b_size) != 0)
			zio->io_error = SET_ERROR(EIO);

		abd_return_linear_copy(zio->io_data, ddata, hdr->b_size);
...
l2arc_release_cdata_buf(arc_buf_hdr_t *ab)
{
	...
	if (l2hdr->b_compress == ZIO_COMPRESS_LZ4) {
		 ...
		 */
		ASSERT(l2hdr->b_tmp_cdata != NULL);
		zio_data_buf_free(ABD_TO_LINEAR(l2hdr->b_tmp_cdata), ab->b_size);
	}
	l2hdr->b_tmp_cdata = NULL;
}
