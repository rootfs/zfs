...
dbuf_evict_user(dmu_buf_impl_t *db)
{
	...
	if (db->db_user_data_ptr_ptr)
		*db->db_user_data_ptr_ptr = ABD_TO_LINEAR(db->db.db_data);
...
dbuf_verify(dmu_buf_impl_t *db)
{
	...
	if (db->db_blkptr) {
		if (db->db_parent == dn->dn_dbuf) {
			...
			if (RW_WRITE_HELD(&dn->dn_struct_rwlock)) {
				ASSERT3P(db->db_blkptr, ==,
				    ((blkptr_t *)ABD_TO_LINEAR(db->db_parent->db.db_data) +
				    db->db_blkid % epb));
			}
		}
	}
	if ((db->db_blkptr == NULL || BP_IS_HOLE(db->db_blkptr)) &&
		 ...
		 */
		if (db->db_dirtycnt == 0) {
			ASSERTV(uint64_t tmp);
			ASSERTV(abd_iterate_rfunc(db->db.db_data, db->db.db_size,
			    checkzero, &tmp));
			ASSERT(tmp == 0);
		}
	}
...
dbuf_read_impl(dmu_buf_impl_t *db, zio_t *zio, uint32_t *flags)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		db->db.db_data = LINEAR_TO_ABD(zio_buf_alloc(DN_MAX_BONUSLEN));
		arc_space_consume(DN_MAX_BONUSLEN, ARC_SPACE_OTHER);
		if (bonuslen < DN_MAX_BONUSLEN)
			bzero(ABD_TO_LINEAR(db->db.db_data), DN_MAX_BONUSLEN);
		if (bonuslen)
			bcopy(DN_BONUS(dn->dn_phys), ABD_TO_LINEAR(db->db.db_data), bonuslen);
		DB_DNODE_EXIT(db);
		dbuf_update_data(db);
		db->db_state = DB_CACHED;
		mutex_exit(&db->db_mtx);
...
dbuf_fix_old_data(dmu_buf_impl_t *db, uint64_t txg)
{
	...
	if (dr == NULL ||
	    (dr->dt.dl.dr_data !=
	    ((db->db_blkid  == DMU_BONUS_BLKID) ? ABD_TO_LINEAR(db->db.db_data) : db->db_buf)))
		return;
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		arc_space_consume(DN_MAX_BONUSLEN, ARC_SPACE_OTHER);
		abd_copy_to_buf(dr->dt.dl.dr_data, db->db.db_data, DN_MAX_BONUSLEN);
	...
	if (db->db_level == 0) {
		...
		if (db->db_state != DB_NOFILL) {
			if (db->db_blkid == DMU_BONUS_BLKID) {
				...
				data_old = ABD_TO_LINEAR(db->db.db_data);
...
dbuf_assign_arcbuf(dmu_buf_impl_t *db, arc_buf_t *buf, dmu_tx_t *tx)
{
	...
	if (db->db_state == DB_CACHED &&
		...
		if (ABD_IS_LINEAR(buf->b_data))
			abd_copy_from_buf(db->db.db_data, ABD_TO_LINEAR(buf->b_data),
			    db->db.db_size);
		else
...
dbuf_clear(dmu_buf_impl_t *db)
{
	...
	if (db->db_state == DB_CACHED) {
		ASSERT(db->db.db_data != NULL);
		if (db->db_blkid == DMU_BONUS_BLKID) {
			zio_buf_free(ABD_TO_LINEAR(db->db.db_data), DN_MAX_BONUSLEN);
    ...
    dmu_buf_impl_t **parentp, blkptr_t **bpp, struct dbuf_hold_impl_data *dh)
{
	...
	*bpp = NULL;
	...
	if (level >= nlevels ||
		...
		*bpp = ((blkptr_t *)ABD_TO_LINEAR((*parentp)->db.db_data)) +
		    (blkid & ((1ULL << epbs) - 1));
		return (0);
	} else {
...
dbuf_check_blkptr(dnode_t *dn, dmu_buf_impl_t *db)
{
	...
	if (db->db_level == dn->dn_phys->dn_nlevels-1) {
		...
		db->db_blkptr = (blkptr_t *)ABD_TO_LINEAR(parent->db.db_data) +
...
dbuf_sync_leaf(dbuf_dirty_record_t *dr, dmu_tx_t *tx)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		if (*datap != ABD_TO_LINEAR(db->db.db_data)) {
...
dbuf_write_ready(zio_t *zio, arc_buf_t *buf, void *vdb)
{
	...
	if (db->db_level == 0) {
		...
		if (dn->dn_type == DMU_OT_DNODE) {
			dnode_phys_t *dnp = ABD_TO_LINEAR(db->db.db_data);
		...
		blkptr_t *ibp = ABD_TO_LINEAR(db->db.db_data);
		ASSERT3U(db->db.db_size, ==, 1<<dn->dn_phys->dn_indblkshift);
