	...
	if (!BP_IS_HOLE(os->os_rootbp)) {
		...
		if (spa_version(spa) >= SPA_VERSION_USERSPACE &&
			...
			(void) arc_buf_remove_ref(os->os_phys_buf,
			    &os->os_phys_buf);
			os->os_phys_buf = ABD_TO_LINEAR(buf);
		}

		os->os_phys = ABD_TO_LINEAR(os->os_phys_buf->b_data);
...
dmu_objset_userquota_find_data(dmu_buf_impl_t *db, dmu_tx_t *tx)
{
	...
	if (db->db_dirtycnt == 0)
		return ABD_TO_LINEAR(db->db.db_data);  /* Nothing is changing */
	...
	if (dr == NULL) {
		...
		if (dn->dn_bonuslen == 0 &&
			...
			data = ABD_TO_LINEAR(dr->dt.dl.dr_data->b_data);
		else
...
dmu_objset_userquota_get_ids(dnode_t *dn, boolean_t before, dmu_tx_t *tx)
{
	...
	if (before && dn->dn_bonuslen != 0)
		...
		if (dn->dn_bonus) {
			...
			ASSERT(error == 0);
			mutex_enter(&db->db_mtx);
			data = (before) ? ABD_TO_LINEAR(db->db.db_data) :
